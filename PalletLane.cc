...
const PalletRow& 
PalletLane::operator [] (uint row) const noexcept {
  return pallet_rows_[row];
}

bool 
PalletLane::put(Pallet::Ptr& pallet, uint row, uint column) noexcept {
...
bool 
PalletLane::isEmpty(uint row, uint column) const noexcept {
  if(row >= row_count) return false;
...
bool 
PalletLane::isAccessible(Height object_height, uint row, uint column) const noexcept {
...
uint 
PalletLane::getNumber() const noexcept {
