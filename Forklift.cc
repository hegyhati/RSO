...
bool Forklift::isLoaded() const noexcept {
  return  load_.get() != nullptr;
}

bool Forklift::load(Pallet::Ptr& pallet) noexcept {
...
bool Forklift::unload(PalletBlock& block, uint lane, uint row, uint column) noexcept {
...
std::ostream& operator << (std::ostream& s, const Forklift& fl) {
